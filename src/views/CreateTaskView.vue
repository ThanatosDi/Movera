...
const handleCreateTask = async () => {
  ...
  try {
    ...
    if (e instanceof ApiError && e.detail.error === 'TaskAlreadyExists') {
      error_message.value = t('exceptions.TaskAlreadyExists', { taskName: createTaskData.value.name })
    } else {
      error_message.value = e instanceof Error ? e.message : 'Unknown error'
    }
...
<template>
  <main class="`flex-1 flex flex-col p-4 space-y-4 overflow-auto pb-6`">
    ...
    <!-- 任務設定卡片 -->
    <Card class="bg-card text-foreground border border-border">
      <CardHeader>
