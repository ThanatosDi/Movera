...
class wsService:
    ...
    async def _handle_create_task(
        ...
        """
        try:
            task = schemas.TaskCreate(**payload)
    ...
    async def _handle_update_task(
        ...
        try:
            ...
            task_update = schemas.TaskUpdate(**payload)
        except Exception as e:
            raise ValueError(f"Invalid task data: {e}")
    ...
    async def _handle_delete_task(self, payload: dict[str, str]) -> None:
        ...
        self.services.task.delete_task(task_id)
    ...
    async def _handle_preview_parse(self, payload: dict[str, str]):
        try:
            payload: schemas.ParsePreviewRequest = schemas.ParsePreviewRequest(
                **payload
            )
        except Exception:
            raise PayloadValidationError(f"Invalid parse preview data: {payload}")
    ...
    async def _handle_preview_regex(self, payload: dict[str, str]):
        try:
            payload: schemas.RegexPreviewRequest = schemas.RegexPreviewRequest(
                **payload
            )
        except Exception:
            raise PayloadValidationError(f"Invalid regex preview data: {payload}")
